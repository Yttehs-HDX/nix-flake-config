...
软件包的分类基于一套四维坐标系，定义在 `modules/package-governance/taxonomy.nix` 中。
本文档是从 `1.md`（原始设计草案）与重构实现中提炼出的正式规范。

---
## 架构原则
`modules/package-governance/presets.nix` 提供以下预设，每个预设生成一条完整的元数据记录。  
...
- loader 支持自动发现 `modules/packages/*/default.nix`
...
### `modules/package-governance/catalog/home.nix`
...
### `modules/package-governance/catalog/system.nix`
...
`modules/package-governance/rules.nix` 提供以下运行时判定函数：
说明：
...
`modules/package-governance/diagnostics.nix` 提供：
...
└── catalog/
  ├── home.nix             # home scope 包元数据注册表（由公共函数派生）
  └── system.nix           # system scope 包元数据注册表（由公共函数派生）
```
```
...
### 消费者
以下模块通过 `import ../package-governance { inherit lib; }` 使用此系统：
...
Backend 实现文件已内聚到 package definition 目录：
- `modules/packages/<packageId>/home.nix`
- `modules/packages/<packageId>/nixos.nix`
- `modules/packages/<packageId>/darwin.nix`

`default.nix` 只负责 metadata 与 backend path 引用，不再承载具体 backend 逻辑。
...
当前统一由公共模块 `modules/projection/common/package-modules.nix` 负责：
- 根据 `(backendType, scope)` 从 definitions 解析 backend 路径
- 将 `input.packages.<scope>` 映射为 module 列表
- 对无实现的 backend path（null）返回 no-op module

```nix
# projection/common/package-modules.nix (shared)
...
1. **创建 package definition 目录**：创建 `modules/packages/<packageId>/`，并初始化 `default.nix` + backend 文件
2. **填写 metadata**：使用 presets 或手写完整元数据
3. **实现 backend projector**：在 `modules/packages/<packageId>/home.nix` / `nixos.nix` / `darwin.nix` 中实现逻辑
