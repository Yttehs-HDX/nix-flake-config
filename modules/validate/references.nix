...
lib.mapAttrsToList
  (relationId: relation:
    if !(builtins.hasAttr relation.user normalized.users) then
      ...
      throw "Enabled relation `${relationId}` cannot target disabled user `${relation.user}`."
    else if relation.enable && !normalized.hosts.${relation.host}.enable then
      throw "Enabled relation `${relationId}` cannot target disabled host `${relation.host}`."
    else
      true)
