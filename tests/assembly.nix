...
assert builtins.elem "wireshark"
  nixosConfig.config.users.users.shetty.extraGroups;
assert homeConfig.config.i18n.inputMethod.type == "fcitx5";
assert homeConfig.config.programs.git.enable;
assert homeConfig.config.programs.direnv.nix-direnv.enable;
assert homeConfig.config.programs.rofi.enable;
assert homeConfig.config.programs.waybar.enable;
...
assert homeConfig.config.wayland.windowManager.hyprland.enable;
